...
const unsigned int SHA_CONTEXT_SIZE = sizeof(sha3_context);
 ...
 */
void
sha3_Finalize(void *priv,  unsigned char *out)
{
  sha3_context *ctx = (sha3_context *) priv;
  ...
  /* capacityWords: 16 for sha512, thus 16*4 = 64 bytes of output */
  memcpy(out, ctx->sb, ctx->capacityWords * (sizeof(uint64_t) / 2));
}
